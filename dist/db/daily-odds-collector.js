...
class DailyOddsCollector {
    constructor() {
        ...
        this.MAX_RETRIES = 3;
        this.RETRY_DELAY = 5000; // 5秒
        this.lastBrowserReset = new Date();
        this.BROWSER_RESET_INTERVAL = 12 * 60 * 60 * 1000; // 12時間ごとにブラウザをリセット
    ...
    async checkAndResetBrowserIfNeeded() {
                            ...
                            for (const row of rows) {
                                ...
                                if (isGrade) {
                                    ...
                                    if (timeText === '発走済') {
                                        ...
                                        const raceId = parseInt(`${year}${venueCode}${kai.padStart(2, '0')}${nichi.padStart(2, '0')}${raceNumber.toString().padStart(2, '0')}`);
    ...
    // オッズ収集のスケジュール設定
    async scheduleOddsCollection(race) {
        try {
            // 既存のジョブがあれば削除
            ...
            if (race.isGrade) {
                ...
                this.logWithTimestamp('info', `Race ${race.id} is grade. Collection will start at ${jstCollectionStartTime} (JST)`);
            }
            // UTC時間をJST表示に変換
            ...
            const job = schedule.scheduleJob('*/5 * * * *', async () => {
                try {
                    ...
                    if (race.isGrade && collectionStartTime && now < collectionStartTime) {
                        ...
                        this.logWithTimestamp('info', `Race ${race.id} is grade. Waiting for collection start time: ${jstCollectionStart} (JST). Current: ${jstNow} (JST)`);
                        return;
                    }
                    // レース開始までの時間（ミリ秒）
    ...
    async checkAndRestoreRaceJobs() {
        try {
            // アクティブなジョブの状態を確認
            this.checkAndRestoreJobs();
            // upcomingレースに対するジョブが存在することを確認
            const upcomingRaces = await this.withDbRetry(() => db.select()
                .from(races)
    ...
    // エラー回復機能を追加
    async recoverFromError(operation) {
    ...
    async collectOdds(raceId) {
        try {
            ...
            const race = await this.withDbRetry(() => db.query.races.findFirst({
                where: eq(races.id, raceId)
            }));
            ...
            if (race.startTime < now && race.status === 'upcoming') {
                ...
                // 最終オッズを取得
                const betTypes = ['tanpuku', 'wakuren', 'umaren', 'wide', 'umatan', 'fuku3', 'tan3'];
            ...
            const oddsTypes = ['tanpuku', 'wakuren', 'umaren', 'wide', 'umatan', 'fuku3', 'tan3'];
    ...
    // 単複オッズ処理を分離
    async handleTanpukuOdds(raceId, odds) {
    ...
    // その他のオッズ処理を分離
    async handleOtherOdds(betType, odds) {
        const updateMethod = {
    ...
    // 古いレースデータを削除する関数を追加
    async cleanupOldRaceData() {
        try {
            // 30日以上前のレースを取得
            const thirtyDaysAgo = new Date();
            thirtyDaysAgo.setDate(thirtyDaysAgo.getDate() - 30);
            const oldRaces = await this.withDbRetry(() => db.select()
                .from(races)
...
async function runWithAutoRestart() {
    ...
    while (true) {
        try {
            ...
            const restartDelay = Math.min(INITIAL_RESTART_DELAY * Math.pow(2, consecutiveFailures - 1), 30 * 60 * 1000 // 最大30分
            );
